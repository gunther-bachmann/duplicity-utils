...
(defun gb/backup--normalize-fs-name-for-prefix-check (fs-name)
...
(defun gb/backup--covered-by-backup-on-rule-line (covered-by-backup file-name-norm)
...
the given FILE-NAME-NORM alized and the current position in the exclude-file"
  (let ((lmin (point)))
    ...
    (let* ((lmax (point))
           ...
           (line-str-norm (gb/backup--normalize-fs-name-for-prefix-check
...
(defun gb/backup--map-files-covered-by-backup (exclusion-config file-name-list)
...
based on the given EXCLUSION-CONFIG"
  (with-temp-buffer
    ...
    (cl-mapcar
     (lambda (file-name-el)  
       (cons
        (let ((file-name-norm (gb/backup--normalize-fs-name-for-prefix-check file-name-el)))          
          (goto-char (point-min))
          (let ((covered-by-backup t))
            (while (not (eobp))
              (setq covered-by-backup
                    (gb/backup--covered-by-backup-on-rule-line covered-by-backup file-name-norm))
              (forward-line 1)
              (beginning-of-line))
            covered-by-backup))
        file-name-el))
...
(ert-deftest gb/check-file-backup-status--plus-folder-rule--parent-folder ()
    ...
    (should (eq 'partial (with-temp-buffer
                           ...
                           (goto-char (point-min))
                           (gb/backup--covered-by-backup-on-rule-line nil "/some/folder/")))))

(ert-deftest gb/check-file-backup-status--plus-file-rule--parent-folder ()
...
(ert-deftest gb/check-file-backup-status--plus-folder-rule--parent-folder-file ()
  "adding a folder which is a subfolder of a folder the current file being checked is in, is still excluded"
  (should-not (with-temp-buffer
                (insert "+ /some/folder/and/more/\n")
                (goto-char (point-min))
                (gb/backup--covered-by-backup-on-rule-line nil "/some/folder/file"))))

;; regular exclusions

(ert-deftest gb/check-file-backup-status--folder-rule--sub-file ()
  "folder will ensure that direct sub files are excluded from backup"
  (should-not (with-temp-buffer
                (insert "/some/folder\n")
                (goto-char (point-min))
                (gb/backup--covered-by-backup-on-rule-line t "/some/folder/sub-file"))))

(ert-deftest gb/check-file-backup-status--folder-rule--deep-sub-file ()
  "folder will ensure that deep sub files are excluded in backup"
  (should-not (with-temp-buffer
                (insert "/some/folder\n")
                (goto-char (point-min))
                (gb/backup--covered-by-backup-on-rule-line t "/some/folder/other/folder/sub-file"))))

(ert-deftest gb/check-file-backup-status--folder-rule--sub-folder ()
  "folder will ensure that direct sub folder are excluded in backup"
  (should-not (with-temp-buffer
                (insert "/some/folder\n")
                (goto-char (point-min))
                (gb/backup--covered-by-backup-on-rule-line t "/some/folder/sub-folder/"))))

(ert-deftest gb/check-file-backup-status--folder-rule--folder ()
  "folder will ensure that this folder is excluded in backup"
  (should-not (with-temp-buffer
                (insert "/some/folder\n")
                (goto-char (point-min))
                (gb/backup--covered-by-backup-on-rule-line t "/some/folder/"))))

(ert-deftest gb/check-file-backup-status--folder-rule--deep-sub-folder ()
  "folder will ensure that deep sub folder are excluded in backup"
  (should-not (with-temp-buffer
                (insert "/some/folder\n")
                (goto-char (point-min))
                (gb/backup--covered-by-backup-on-rule-line t "/some/folder/sub-folder/and/other/"))))

;; plus rules

(ert-deftest gb/check-file-backup-status--plus-folder-rule--sub-file ()
    "adding a folder will ensure that direct sub files are included in backup"
  (should (with-temp-buffer
            (insert "+ /some/folder\n")
            (goto-char (point-min))
            (gb/backup--covered-by-backup-on-rule-line nil "/some/folder/sub-file"))))

(ert-deftest gb/check-file-backup-status--plus-folder-rule--deep-sub-file ()
    "adding a folder will ensure that deep sub files are included in backup"
  (should (with-temp-buffer
            (insert "+ /some/folder\n")
            (goto-char (point-min))
            (gb/backup--covered-by-backup-on-rule-line nil "/some/folder/other/folder/sub-file"))))

(ert-deftest gb/check-file-backup-status--plus-folder-rule--sub-folder ()
    "adding a folder will ensure that direct sub folder are included in backup"
  (should (with-temp-buffer
            (insert "+ /some/folder\n")
            (goto-char (point-min))
            (gb/backup--covered-by-backup-on-rule-line nil "/some/folder/sub-folder/"))))

(ert-deftest gb/check-file-backup-status--plus-folder-rule--folder ()
...
(ert-deftest gb/check-file-backup-status--plus-folder-rule--deep-sub-folder ()
    "adding a folder will ensure that deep sub folder are included in backup"
  (should (with-temp-buffer
            (insert "+ /some/folder\n")
            (goto-char (point-min))
            (gb/backup--covered-by-backup-on-rule-line nil "/some/folder/sub-folder/and/other/"))))

(defun gb/check-current-file-backup-status ()
  (interactive)
  (when (eq major-mode 'dired-mode)
    (ignore-errors
      (let ((file-name (gb/backup--get-file-name-at-point)))
        (when (or (file-exists-p file-name) (file-directory-p file-name))
          (when-let ((found (cl-find-if
                             (lambda (pair) (string-equal
                                        (gb/backup--normalize-fs-name-for-prefix-check file-name)
                                        (cdr pair)))
                             (gb/backup--map-files-covered-by-backup
                              "~/.duplicity/data.exclude.list.txt"
                              (list (gb/backup--normalize-fs-name-for-prefix-check file-name))))))
            (cond ((eq t (car found))
                   (pulsar-pulse-line)
                   (message "backed up"))
...
(bind-key "C-c b i" #'gb/check-current-file-backup-status dired-mode-map)
      ...
      (save-excursion
        ...
        (while (not (eobp))
          ...
          (ignore-errors
            (let ((file-name (gb/backup--get-file-name-at-point)))
              (when (or (file-exists-p file-name) (file-directory-p file-name))
                (setq file-name-list (cons (gb/backup--normalize-fs-name-for-prefix-check file-name) file-name-list)))))
          (forward-line)
          ))
      file-name-list)))
      ...
      (save-excursion
        ...
        (while (not (eobp))
          (ignore-errors
            ...
            (let ((file-name (gb/backup--get-file-name-at-point)))
              (when (or (file-exists-p file-name) (file-directory-p file-name))
                (when-let ((stat (gb/backup--get-backup-status-from-definition file-name)))
                  (cond ((eq t stat)
...
(defun gb/backup--file-status (file-name)  
  (let ((abs-file-name (expand-file-name file-name)))
    (if (not (file-exists-p (format "/run/media/%s/%s/data-backup" (user-login-name) gb/backup-mount)))
...
(defun gb/backup--note-popup (position text)
  ...
  (let ((buffer (get-buffer-create "* backup file status *")))
    ...
    (add-hook 'pre-command-hook #'gb/backup--remove-note-popup)))

(defun gb/backup--get-file-name-at-point ()
  (call-interactively #'diredp-copy-abs-filenames-as-kill)
  diredp-last-copied-filenames)

(defun gb/backup--remove-note-popup ()
  "called to remove popup"
  (remove-hook 'pre-command-hook #'gb/backup--remove-note-popup)
  (posframe-hide-all))

(defun gb/backup--find-backup-status-from-definition (file-name)
...
(defun gb/backup--get-backup-status-from-definition (file-name)
  "try to find the backup status only for the given FILE-NAME based on the backup definition"
  (if-let ((found (gb/backup--find-backup-status-from-definition file-name)))
      (car found)
    nil))

(defun gb/backup--show-current-file-backup-info ()
  "show actual backup information for the file at point"
  (interactive)
  (when (eq major-mode 'dired-mode)
    (ignore-errors
      (let ((file-name (gb/backup--get-file-name-at-point))) ;; dired-x-guess-file-name-at-point
        (when (file-exists-p file-name)
          (gb/backup--note-popup (point) (gb/backup--file-status file-name)))))))

(bind-key "C-c b b" #'gb/backup--show-current-file-backup-info dired-mode-map)

(defun gb/backup--show-probable-backup-status ()
    ...
    (let ((file-name (gb/backup--get-file-name-at-point))) ;; dired-x-guess-file-name-at-point
      (when (file-exists-p file-name)
        (let ((file-access-str (format-time-string "%Y-%m-%d %T" (file-attribute-modification-time (file-attributes file-name))))
          ...
          (gb/backup--note-popup
           (point)
           (format "%s\n(backup date %s, file date %s)"
                   (let ((is-backed-up (gb/backup--get-backup-status-from-definition file-name)))
